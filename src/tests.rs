...
fn operations_uint() {
    ...
    let output = process_and_unwrap_value(
        &mut compiler,
        "10_000_000_000_000_000_000 == 10_000_000_000_000_000_000",
    );
    assert_eq!(output, Value::Bool(true));
    ...
    let output = process_and_unwrap_value(
        &mut compiler,
        "10_000_000_000_000_000_001 > 10_000_000_000_000_000_000",
    );
    assert_eq!(output, Value::Bool(true));

    let output = process_and_unwrap_value(
        &mut compiler,
        "10_000_000_000_000_000_001 >= 10_000_000_000_000_000_002",
    );
    assert_eq!(output, Value::Bool(false));

    let output = process_and_unwrap_value(&mut compiler, "10_000_000_000_000_000_000 + 7_uint");
    assert_eq!(output, Value::Uint(10_000_000_000_000_000_007));

    let output = process_and_unwrap_value(&mut compiler, "10_000_000_000_000_000_015 - 5_uint");
    assert_eq!(output, Value::Uint(10_000_000_000_000_000_010));

    let output = process_and_unwrap_value(&mut compiler, "7_uint * 8_uint");
    assert_eq!(output, Value::Uint(56));

    let output = process_and_unwrap_value(&mut compiler, "7_uint / 3_uint");
    assert_eq!(output, Value::Uint(2));

    let output = process_and_unwrap_value(&mut compiler, "7_uint % 3_uint");
    assert_eq!(output, Value::Uint(1));
}
...
fn operations_string() {
    ...
    let output = process_and_unwrap_value(&mut compiler, "\"abc\" * 3_uint");
    assert_eq!(output, Value::Str(Box::new(String::from("abcabcabc"))));

    let output = process_and_unwrap_value(&mut compiler, "3_uint * \"abc\"");
    assert_eq!(output, Value::Str(Box::new(String::from("abcabcabc"))));
}
...
fn number_parsing() {
    ...
    let output = process_and_unwrap_value(&mut compiler, "10_000_000_000_000_000_000");
    assert_eq!(output, Value::Uint(10000000000000000000));
    ...
    let output = process_and_unwrap_value(&mut compiler, "11_int");
    assert_eq!(output, Value::Int(11));

    let output = process_and_unwrap_value(&mut compiler, "11_uint");
    assert_eq!(output, Value::Uint(11));

    let output = process_and_unwrap_value(&mut compiler, "11_float");
    assert_eq!(output, Value::Float(11.0));
}
